...
public class AprilTag {
    ...
    public void init(HardwareMap hwMap)
    {
        ...
        aprilTag = new AprilTagProcessor.Builder()
                ...
                .setLensIntrinsics(1437.97, 1437.97, 775.085, 623.832)
    ...
    public void showAprilTagDetails(Telemetry telemetry)
    {
        ...
        for (AprilTagDetection detection : currentDetections) {
            if (detection.metadata != null) {
                ...
                telemetry.addLine(String.format("RP XYZ %6.1f %6.1f %6.1f  (inch)", detection.robotPose.getPosition().x, detection.robotPose.getPosition().y, detection.robotPose.getPosition().z));
            } else {
    ...
    public double getDistanceFromRed(Telemetry telemetry)
    {
        double distance = 0.0;
        List<AprilTagDetection> currentDetections = aprilTag.getDetections();
        ...
        if (!currentDetections.isEmpty())
        {
            for (AprilTagDetection detection : currentDetections)
            {
                if (detection.metadata != null)
                {
                    if (detection.metadata.name.equals("RedTarget"))
                    {
                        //telemetry.addData("Red Target", "FOUND");
                        //telemetry.addData("Red Target Distance", detection.ftcPose.y);
    ...
    public double getAngleFromRed(Telemetry telemetry)
    {
        double angle = 999.0;
        List<AprilTagDetection> currentDetections = aprilTag.getDetections();
    ...
    public double getDistanceFromBlue(Telemetry telemetry)
    {
        double distance = 0.0;
        List<AprilTagDetection> currentDetections = aprilTag.getDetections();
        ...
        if (!currentDetections.isEmpty())
        {
            for (AprilTagDetection detection : currentDetections)
            {
                if (detection.metadata != null)
                {
                    if (detection.metadata.name.equals("BlueTarget"))
                    {
                        //telemetry.addData("Blue Target", "FOUND");
                        //telemetry.addData("Blue Target Distance", detection.ftcPose.y);
                        distance =  detection.ftcPose.y;
                        break;
                    }
                }
        ...
        {
            //telemetry.addData("Blue Target", "NOT FOUND");
        }

        return distance;
    }

    public double getAngleFromBlue(Telemetry telemetry)
    {
        double angle = 999.0;
        List<AprilTagDetection> currentDetections = aprilTag.getDetections();
    ...
    public ObeliskPattern getObeliskPattern(Telemetry telemetry)
    {
        List<AprilTagDetection> currentDetections = aprilTag.getDetections();
